...
import numpy as np
import tqdm
from snn_conv_formula3 import SinDense
import torch as t
import pandas as pd
...
sd1 = SinDense(16,last_input_condition = True,init_weights_range_in = [-0.1,0.1],init_weights_range_out = [-0.1,0.1])
sd2 = SinDense(10,last_input_condition = True,init_weights_range_in = [-0.1,0.1],init_weights_range_out = [-0.1,0.1])
...
for epoch in range(1): #epoch
    ...
    cnta = 0
    max_errors = []
    for cnt in range(0,60000,1):
        out1 = sd1.forward(data[cnt])
        out2 = sd2.forward(out1)
        
        main_error = (1/2)*(out2-labels[cnt])**2
        main_error_t = t.tensor(out2-labels[cnt])
        derror_dout = main_error*(out2-labels[cnt])
        
        error1 = sd2.backprop(derror_dout.unsqueeze(0),out1,learning_rate = lr)
